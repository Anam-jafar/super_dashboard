...
@section('content')
  <div class="main-content app-content">
    <div class="container-fluid">
      ...
      <form action="{{ route('metrix.settings.store', ['type' => 'fidyah']) }}" method="POST" class="space-y-8">
        @csrf
          ...
          <div id="individual_statuses" class="space-y-4">
            <div class="status-group">
              <div class="flex gap-4">
                <input type="text" name="individual_status[0][parameter]" placeholder="Status Parameter" required
                  class="flex-1 rounded-md border-gray-300 px-3 py-2 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                <button type="button" onclick="addCategory(this)"
                  class="rounded-md bg-blue-100 px-4 py-2 text-blue-500">Add Category</button>
                <button type="button" onclick="removeStatus(this)" class="px-4 py-2 text-red-500">X</button>
              </div>
              <div class="categories mt-3 space-y-3">
                <div class="flex gap-4">
                  <input type="text" name="individual_status[0][categories][0][parameter]"
                    ...
                    class="flex-1 rounded-md border-gray-300 px-3 py-2 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                  <button type="button" onclick="removeCategory(this)" class="px-4 py-2 text-red-500">X</button>
                </div>
              </div>
            </div>
          </div>
          <button type="button" onclick="addStatus()" class="mt-2 rounded-md bg-indigo-100 px-4 py-2 text-indigo-700">Add
          ...
          <div id="fidyah_items" class="space-y-3">
            <div class="flex gap-4">
              ...
              <input type="number" name="fidyah_item[0][price]" placeholder="Price" required
                class="w-32 rounded-md border-gray-300 px-3 py-2 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
              <button type="button" onclick="removeItem(this)" class="px-4 py-2 text-red-500">X</button>
            </div>
          </div>
          <button type="button" onclick="addFidyahItem()"
            class="mt-2 rounded-md bg-indigo-100 px-4 py-2 text-indigo-700">Add Item</button>
        </div>
        ...
        <div class="space-y-2">
          ...
          <input type="number" name="rate" id="rate" step="0.01" required
            class="block w-96 rounded-md border-gray-300 px-3 py-2 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
            placeholder="Enter the rate">
        </div>

        <!-- Submit Button -->
        <div class="text-right">
          <button type="submit" class="rounded-md bg-indigo-600 px-4 py-2 text-white hover:bg-indigo-700">Submit</button>
        </div>
      </form>
    </div>
  </div>
