    <form method="GET" action="{{ $route }}" class="flex items-center justify-between w-full gap-4">
        <div class="flex items-center flex-1 gap-4">
            @foreach ($filters as $filter)
                @if ($filter['type'] === 'select')
                    <select id="{{ $filter['name'] }}" name="{{ $filter['name'] }}"
                        class="ti-form-select rounded-sm py-2 px-3 w-40">
                        <option value="" {{ request($filter['name']) == '' ? 'selected' : '' }}>All
                            {{ $filter['label'] }}</option>
                ...
                @elseif ($filter['type'] === 'text')
                    <input type="text" id="{{ $filter['name'] }}" name="{{ $filter['name'] }}"
                        value="{{ request($filter['name']) }}" placeholder="{{ $filter['placeholder'] ?? '' }}"
                        class="ti-form-input rounded-sm py-2 px-3 w-40">
                @endif
            @endforeach
        ...
        <!-- Submit Button -->
        <button type="submit" class="ti-btn ti-btn-primary btn-wave waves-effect waves-light ti-btn-w-lg ti-btn-lg"
