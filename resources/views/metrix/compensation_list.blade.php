            ...
            <div class="max-w-full mx-auto mt-4 p-4 sm:p-6 lg:p-8 bg-gray-50 min-h-[80vh]">
                <div class="flex flex-col sm:flex-row justify-between items-center mb-4 space-y-2 sm:space-y-0">
                    ...
                    <a href="{{ route('compensation.create') }}"
                        class="bg-blue-600 text-white px-4 py-1 rounded-full hover:bg-blue-700 transition duration-300 ease-in-out transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 shadow-lg">
                ...
                @if (session('success'))
                    <div
                        class="p-4 mb-4 text-green-700 bg-green-100 border-l-4 border-green-500 rounded-r-lg animate-fade-in-down">
                        {{ session('success') }}
                    </div>
                @elseif (session('error'))
                    <div
                        class="p-4 mb-4 text-red-700 bg-red-100 border-l-4 border-red-500 rounded-r-lg animate-fade-in-down">
                        {{ session('error') }}
                    </div>
                @endif
                ...
                <div class="space-y-4">
                    @foreach ($payment_metrix as $index => $setting)
                        <div
                            class="bg-white shadow-lg rounded-lg overflow-hidden transition duration-300 ease-in-out transform hover:shadow-xl">
                            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center p-4 cursor-pointer"
                                ...
                                @if (!$setting['is_active'])
                                    <form action="{{ route('compensation.markAsActive', (string) $setting['_id']) }}"
                                        method="POST" class="mt-3 sm:mt-0">
                                        @csrf
                                        @method('POST')
                                        <button type="submit"
                            ...
                            <div id="card-{{ $index }}" class="hidden">
                                <div class="px-4 pb-4 space-y-4">
                                    ...
                                    <div>
                                        <p class="text-sm text-gray-700"><span class="font-semibold">Rate:</span>
                                            {{ $setting['rate'] }}</p>
                                    </div>
                                </div>
                            </div>
