                  ...
                  @foreach ($status['categories'] as $categoryIndex => $category)
                    <div class="flex gap-4">
                      <input type="text"
                        ...
                        value="{{ $category['parameter'] }}" placeholder="Category Parameter" required
                        class="flex-1 rounded-md border-gray-300 px-3 py-2 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                      <input type="text"
        ...
        <div class="space-y-2">
          ...
          <div id="fidyah_items" class="space-y-3">
            @foreach ($setting['fidyah_item'] as $itemIndex => $item)
              <div class="flex gap-4">
                <input type="text" name="fidyah_item[{{ $itemIndex }}][name]" value="{{ $item['name'] }}"
                  placeholder="Item Name" required
                  class="flex-1 rounded-md border-gray-300 px-3 py-2 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                <input type="number" name="fidyah_item[{{ $itemIndex }}][price]" value="{{ $item['price'] }}"
