            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-4 p-4">
                    ...
                    <div class="flex justify-between items-start">
                        <div>
                            <h2 class="text-sm font-medium opacity-90 uppercase tracking-wide mb-2">
                                {{ $data['number_of_mosque'][0] }} <!-- Access Title -->
                            </h2>
                            <p class="text-4xl font-bold">{{ $data['number_of_mosque'][1] }}</p> <!-- Access Value -->
                        </div>
                ...
                <div class="bg-emerald-600 rounded-lg p-6 text-white relative overflow-hidden">
                    <div class="flex justify-between items-start">
                        <div>
                            <h2 class="text-sm font-medium opacity-90 uppercase tracking-wide mb-2">
                                {{ $data['number_of_kariah'][0] }} <!-- Access Title -->
                            </h2>
                            <p class="text-4xl font-bold">{{ number_format($data['number_of_kariah'][1]) }}</p> <!-- Access Value -->
                        </div>
                        <svg class="w-8 h-8 opacity-80" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                ...
                <div class="bg-orange-500 rounded-lg p-6 text-white relative overflow-hidden md:col-span-2 lg:col-span-2">
                    <div class="flex justify-between items-start">
                        <div>
                            <h2 class="text-sm font-medium opacity-90 uppercase tracking-wide mb-2">
                                {{ $data['number_of_mosque_staff'][0] }} <!-- Access Title -->
                            </h2>
                            <p class="text-4xl font-bold">{{ number_format($data['number_of_mosque_staff'][1]) }}</p> <!-- Access Value -->
                        </div>
                        <svg class="w-8 h-8 opacity-80" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        // Replace the following with the dynamic data passed from the backend
        const pieCharts = @json($pieCharts);

        // Check if the specific chart exists
        const chartData = pieCharts['percentage_of_mosque_type'];
        if (!chartData) {
            console.error('Chart "percentage_of_mosque_type" not found!');
            return;
        }

        // Create a pie chart for "percentage_of_mosque_type"
